...
int dfs(TreeNode *root, int &res) {
    ...
    int leftMaxSum = max(0, dfs(root->left, res));
    int rightMaxSum = max(0, dfs(root->right, res));

    res = max(res, root->val + leftMaxSum + rightMaxSum);
...
//* Driver code
int main() {
    vector<string> nums = {"-10", "9", "20", "null", "null", "15", "7"};
